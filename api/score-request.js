...
//
// Also computes an internal fit score (separate from the lead-facing 0-100
// score below), persists the submission to the `inbound_leads` Supabase
// table, and — for hot/warm leads with a phone number — kicks off the SMS
// nurture sequence (pre-call video + booking link) via Twilio. See
// lib/inboundLeads.js and lead-gen/database/inbound_leads_schema.sql.
// Requires SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY (same project as the
// lead-gen repo) and TWILIO_ACCOUNT_SID/TWILIO_AUTH_TOKEN/TWILIO_FROM_NUMBER
// as additional Vercel env vars — all optional at runtime (enrichment/nurture
// is skipped, not a hard failure, if they're unset, same fail-open pattern
// RESEND_API_KEY already uses below).
...
async function computeScore({ url, gbp }) {
  ...
  return { total: items.reduce((sum, i) => sum + i.points, 0), items, signals };
}
...
module.exports = async function handler(req, res) {
  ...
  const { total, items, signals } = await computeScore({ url, gbp });

  // Enrichment + persistence + nurture kickoff. Entirely best-effort: any
  // failure here (missing env vars, a transient Supabase/Twilio error) must
  // never break the score response the visitor is waiting on, so the whole
  // thing is wrapped and swallowed after logging.
  try {
    const phoneNormalized = normalizePhone(phone);
    const fit = computeFitScore({ business, email, phoneNormalized, url, signals });

    const insertResult = await insertInboundLead({
      name,
      business,
      url: url || null,
      email,
      ...
      public_score: total,
      public_score_breakdown: { items },
      fit_score: fit.fit_score,
      fit_tier: fit.fit_tier,
      fit_flags: fit.fit_flags,
      status: (fit.fit_tier === 'hot' || fit.fit_tier === 'warm') && phoneNormalized ? 'nurturing' : 'enriched',
    });

    if (insertResult.outcome === 'inserted' && phoneNormalized && (fit.fit_tier === 'hot' || fit.fit_tier === 'warm')) {
      const calendlyLink = buildCalendlyLink({ id: insertResult.id, name, email });
      const firstName = name.split(' ')[0] || name;
      const smsResult = await sendSms({
        to: phoneNormalized,
        body: `Hey ${firstName}, it's Haider from A2H — got your score request for ${business}. Made you a quick video before we chat -> ${PRECALL_VIDEO_URL} Grab a time here when you're ready: ${calendlyLink}`,
      });
      if (smsResult.ok) {
        await updateInboundLead(insertResult.id, {
          sms_sequence_step: 1,
    ...
    console.error('inbound lead enrichment/nurture failed:', err);
  }
