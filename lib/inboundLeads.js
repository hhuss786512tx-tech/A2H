...
// request handler.
//
// Wired in from api/mockup-request.js (lead capture -> insertInboundLead),
// api/cron/process-nurture.js (follow-up cadence on existing rows), and
// api/booking-confirmed.js (Calendly redirect handler).
//
// Nurture touches go out over email (Resend), not SMS — this avoids a
// separate Twilio account/phone number/per-message cost. Requires these
// Vercel env vars:
//   SUPABASE_URL
//   SUPABASE_SERVICE_ROLE_KEY
//   RESEND_API_KEY (already required by api/mockup-request.js)
...
const NURTURE_FROM_ADDRESS = 'A2H <hello@a2h.info>';

/**
 * Sends one nurture-sequence email via Resend's REST API directly (no SDK
 * — consistent with this repo's zero-runtime-dependency style for its
 * other integrations). Returns { ok, status, detail? } — never throws, so
 * a failed send doesn't take down the caller's request.
 */
async function sendNurtureEmail({ to, subject, html }) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) return { ok: false, status: 0, detail: 'resend_not_configured' };

  try {
    const res = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
      body: JSON.stringify({ from: NURTURE_FROM_ADDRESS, to, subject, html }),
    });
    if (!res.ok) {
