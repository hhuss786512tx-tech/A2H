...
// api/cron/process-nurture.js (scheduled nudge/reminder/closer cadence),
// api/sms-inbound.js (live two-way AI conversation when a lead texts back),
// and api/booking-confirmed.js (Calendly redirect handler).
//
...
/** Looks up the most recent lead by normalized phone — used by api/sms-inbound.js to match an inbound text to its lead. */
async function getLeadByPhone(phoneNormalized) {
  const supabase = getSupabaseClient();
  if (!supabase || !phoneNormalized) return null;
